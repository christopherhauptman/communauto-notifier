...
const pushover = new Pushover({
  user: 'uztcugfjt2gv7pkg3b4n3ar7vbm82r',
  token: 'a1xc177jz2ohtzppyi6bh2354n9spo',
});
...
// Force check every 5 seconds
const pause = 5;

const distanceRadii = [
  10000, 8000, 6000, 5000, 4000,
  3000, 2000, 1500, 1000, 900,
...
while (true) {
  ...
  if (filteredCars.length) {
    ...
    const message = `${car.brand} ${car.model} is ${Math.floor(car.distance)}m away`;

    console.log(`🚗 Car found! ${message}`);

    // Local desktop notification
    notifier.notify({
      title: 'Communauto Car Found!',
      message,
      sound: true,
    });
    ...
    pushover.send({
      title: 'Communauto Car Found!',
      message,
      sound: 'magic', // You can try other Pushover sounds too
      priority: 1,
    }, (err, result) => {
      if (err) {
...
function getLocation() {
  ...
  const result = execSync('/usr/libexec/geoclue-2.0/demos/where-am-i -t 6').toString();
